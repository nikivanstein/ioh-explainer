...
from ioh_xplainer import __version__, compare, explainer
...
def test_analyse_best():
    ...
    # test analyse best
    test_explainer.analyse_best(
        filename=None, check_bias=False, full_run=True, full_run_folder="output", reps=1
    )

    # test get_results_for_config
    res = test_explainer.get_results_for_config(
        {"popsize": 5, "strategy": "best1bin"}, dim=5, fid=1, iid=1
    )
    assert len(res) > 0
